...
async function savePick(event) {
  ...
  const record = {
    ...
    fair_odds: event.fair_odds ?? null,
    market_probability: event.market_probability ?? null,
    edge: event.edge ?? null,
    expected_value: event.expected_value ?? null,
    kelly_fraction: event.kelly_fraction ?? 0,
    suggested_stake_units: event.suggested_stake_units ?? 0,
    risk_score: event.risk_score ?? null,
    risk_level: event.risk_level || "",
    risk_flags: event.risk_flags || [],
    professional_score: event.professional_score ?? event.value_score ?? 0,
...
function slimEvent(event) {
  return {
    ...
    professional_score: event.professional_score,
    expected_value: event.expected_value,
    fair_odds: event.fair_odds,
    market_probability: event.market_probability,
    kelly_fraction: event.kelly_fraction,
    suggested_stake_units: event.suggested_stake_units,
